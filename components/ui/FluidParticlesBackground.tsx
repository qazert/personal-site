...
/* Classic Perlin noise. Particles read their heading off it rather than
   drifting randomly, so the field moves as one fluid instead of a swarm. */
...
/**
 * A field of drifting dots, steered by Perlin noise rather than randomness, so
 * it reads as one fluid instead of a swarm. Fills its own parent rather than
 * the viewport, so it can sit behind a single section instead of the whole
...
export function FluidParticlesBackground({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      frame += 1;
      if (!stopped && !reduce) requestAnimationFrame(draw);
    };

    /* Reduced motion still gets a few settled passes, so the field isn't
       empty, just still. */
    if (reduce) {
